...
def convert_obj_dataset_to_urdfs_abs_path(obj_ds_path, texture_size=(1024, 1024), n_faces=None):
    obj_dataset = BOPObjectDataset(obj_ds_path)
    urdf_dir = obj_ds_path / 'urdfs'
    urdf_dir.mkdir(exist_ok=True, parents=True)
    for n in tqdm(range(len(obj_dataset))):
        obj = obj_dataset[n]
        ply_path = Path(obj['mesh_path'])
